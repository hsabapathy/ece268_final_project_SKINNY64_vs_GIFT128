...
static void run_ctr_nonce_test()
{
    printf("\n=== CTR Nonce Reuse Attack Demo ===\n");
    // Two different messages encrypted under the same key+nonce
    // XORing ciphertexts reveals XOR of plaintexts, demonstrating why nonce must be unique
    ...
    const char *msg1 = "Attack at dawn!!";
    const char *msg2 = "Defend at night!";
    size_t len = 16;

    uint8_t ct1[16], ct2[16], xored_ct[16];
    gift128_ctr_crypt(key, nonce, (const uint8_t*)msg1, ct1, len);
    gift128_ctr_crypt(key, nonce, (const uint8_t*)msg2, ct2, len);

    for (size_t i = 0; i < len; i++) xored_ct[i] = ct1[i] ^ ct2[i];

    // XOR of ciphertexts == XOR of plaintexts
    uint8_t xored_pt[17] = {};
    for (size_t i = 0; i < len; i++) xored_pt[i] = ((uint8_t*)msg1)[i] ^ ((uint8_t*)msg2)[i];

    printf("Msg1: \"%s\"\n", msg1);
    printf("Msg2: \"%s\"\n", msg2);
    print_hex("CT1 XOR CT2", xored_ct, len);
    print_hex("PT1 XOR PT2", xored_pt, len);
    bool leaked = (memcmp(xored_ct, xored_pt, len) == 0);
    printf("CT XOR == PT XOR (nonce reuse leaks data): %s\n\n", leaked ? "YES (dangerous!)" : "NO");
}
