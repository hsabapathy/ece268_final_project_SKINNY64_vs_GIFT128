...
// CPU key-schedule helpers (key setup only)

// Pre-compute all 40 round-key nibble states on the CPU once.
// The result is uploaded to GPU global memory and reused by all threads,
// eliminating per-thread key-schedule work during CBC decryption.
...
// Throughput sweep
//
// Sweeps 9 input sizes: 1 KB to 64 MB.
// Reports kernel-only wall-clock time and GB/s for:
//   GPU CTR encrypt, GPU CBC decrypt.
// Median of TRIALS (31) measurements per size.
// ===========================================================================
...
{
    printf("\n");
    printf("   GIFT-128 Throughput vs Input Size                             \n");
    printf("   Modes: GPU-CTR encrypt, GPU-CBC-decrypt                       \n");
    printf("  GPU timing: kernel execution only (no H2D/D2H transfers).\n");
    printf("  Throughput denominator: actual data bytes (num_blocks x %d).\n", BLOCK_BYTES);
    ...
    printf("size_bytes,mode,wall_ms,throughput_GBs\n");
    ...
    for (int si = 0; si < NSIZES; si++) {
        ...
        // Build valid CBC ciphertext (sequential kernel, run once for setup)
        gpu_cbc_encrypt(key, iv, h_plain, h_cipher, padded);

        // Device buffers
        uint8_t *d_key_g, *d_nonce_g, *d_rkeys_g, *d_iv_g;
        uint8_t *d_in, *d_cipher_g, *d_out;
        ...
        for (int i = 0; i < TRIALS-1; i++)
            for (int j = i+1; j < TRIALS; j++) {
                if (ctr_samples[j] < ctr_samples[i]) {
                    float t = ctr_samples[i]; ctr_samples[i] = ctr_samples[j]; ctr_samples[j] = t;
                }
                if (cbc_samples[j] < cbc_samples[i]) {
                    float t = cbc_samples[i]; cbc_samples[i] = cbc_samples[j]; cbc_samples[j] = t;
                }
            }
        double ctr_ms  = ctr_samples[TRIALS/2];
        double cbc_ms  = cbc_samples[TRIALS/2];

        double ctr_gbs = (double)N / (ctr_ms * 1e-3) / 1e9;
        double cbc_gbs = (double)N / (cbc_ms * 1e-3) / 1e9;

        const char *unit = (N >= 1024*1024) ? "MB" : "KB";
        double      nd   = (N >= 1024*1024) ? N / 1048576.0 : N / 1024.0;

        printf("  %5.0f %-3s  GPU-CTR      %10.4f ms  %8.4f GB/s\n",
               nd, unit, ctr_ms, ctr_gbs);
        printf("  %5.0f %-3s  GPU-CBC-D    %10.4f ms  %8.4f GB/s\n",
               nd, unit, cbc_ms, cbc_gbs);
        printf("\n");

        printf("%zu,GPU-CTR,%.6f,%.6f\n",  N, ctr_ms, ctr_gbs);
        printf("%zu,GPU-CBC-D,%.6f,%.6f\n",N, cbc_ms, cbc_gbs);
                                         ...
                                         const uint8_t  iv[BLOCK_BYTES])
{
    ...
    for (int t = 0; t < LAT_TRIALS; t++) {
        ...
        ctr_samples[t] = ms * 1000.0f;   // → microseconds
    }

    CUDA_CHECK(cudaMemcpy(d_cipher, h_cipher, sz, cudaMemcpyHostToDevice));
    gift128_cbc_decrypt_kernel<<<1, LAT_BLOCKS>>>(d_rkeys, d_iv, d_cipher, d_out, LAT_BLOCKS);
    CUDA_CHECK(cudaMemcpy(h_recov, d_out, sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaDeviceSynchronize());
...
static void measure_key_schedule_cost(const uint8_t *key)
{
    ...
    // CPU compute cost: time gift128_precompute_round_keys()
    ...
    for (int t = 0; t < KS_TRIALS; t++) {
        ...
        CUDA_CHECK(cudaEventSynchronize(stop));
        float ms = 0; CUDA_CHECK(cudaEventElapsedTime(&ms, start, stop));
        ks_samples[t] = ms * 1000.0f;   // microseconds
    }

    // GPU upload cost: cudaMemcpy of the pre-computed keys H→D
    uint8_t *d_rkeys;
    CUDA_CHECK(cudaMalloc(&d_rkeys, GIFT_ROUNDS * 32));
